...
void NextNextLvl::playGameLvLNext(int score,int ammo,sf::Text Live,sf::Vector3i mana,int force,sf::Clock Animuj)
{
	int w=0;
	int s=0;
	bool fire=false;
	sf::Music Muzyka;
	Muzyka.openFromFile("Data/Music/next.wav");
	sf::Texture bull,bull1,bull2;
	...
	ending.setCharacterSize(60);
	ending.setString("~FINISH~");
	ending.setColor(Color::Red);
	ending.setFont(font);
	ending.setPosition(3700,220);
	introd.setString("Welcone to LEVEL 3");
	...
	poin.loadFromFile("Data/Anim/point3.png");
	...
	sf::Texture bg;
	bg.loadFromFile("Data/Images/BGgame3.png");
	sf:: Sprite Bg;
	Bg.setTexture(bg);
	sf::Texture plat;
	plat.loadFromFile("Data/Images/platform3.png");
	// Platformy
	sf::Sprite platform;
	...
	sf::Texture end;
	end.loadFromFile("Data/Anim/fire.png");
	sf::Sprite End;
	End.setTexture(end);
	End.setPosition(3700,360);
	...
	std::vector<sf::Sprite> bulletVec6;
	...
	while(Okno.isOpen())
	{
		...
		// Strzelanie
			...
			End.setTextureRect(sf::IntRect(klatka*200,linia*200,200,200));
			...
			if (timer.asSeconds() >= 2.0f )
			{
				...
				newBullet.setPosition(2900,500);
			...
			if ( 4==4)
			{
			sf::Time timu = zegar5.getElapsedTime();
			 sf::Sprite newBullet;
			 newBullet.setTexture(bull2);
			 newBullet.setTextureRect(sf::IntRect(klatka*50,linia*50,50,50));
			if (timu.asSeconds() >= 2.0f )
			{
				
				newBullet.setPosition(2900,500);
				bulletVec6.push_back(newBullet);
				zegar5.restart();
			}
			}
			//ATAK PRZECIWNIK 2
		...
		HP = wrog.GetHp();
		stringstream oo,ll,kk;
		kk << score;
		oo << HP;
		ll << HPP;
		Score.setString(kk.str());
...
// KOLIZJE!! KOLIZJE !! KOLIZJE!! KOLIZJE !!			
			...
			for (int i = 1; i < bulletVec3.size();i++) 
		{
			 ...
			 else
			 {
				 ...
				 bulletVec3[i].move(-12,0);
				 if(bulletVec3[i].getGlobalBounds().intersects(player.getGlobalBounds()))
				 {
					
					 player.move(sf::Vector2f(-5,0));
					 cam.move(sf::Vector2f(-5,0));
					  Score.move(sf::Vector2f(-5,0));
					   Live.move(sf::Vector2f(-5,0));
					    Score2.move(sf::Vector2f(-5,0));
						Ammo.move(sf::Vector2f(-5,0));
						Ammo2.move(sf::Vector2f(-5,0));
						Time.move(sf::Vector2f(-5,0));
						czas.move(sf::Vector2f(-5,0));
						  player.Hurt(Live);
			 ...
			 else
			 {
				 ...
				 if(bulletVec4[i].getGlobalBounds().intersects(player.getGlobalBounds()))
				 {
					 ...
					 bulletVec4[i].setColor(Color::Transparent);
			 		 ...
			 		 for (int i = 1; i < bulletVec6.size();i++) 
			 {
			 if (HP<=0)
				 {
					 bulletVec6[i].setColor(Color::Transparent);
					 bulletVec6[i].setPosition(0,900);

				 }
			 else
			 {
				 Okno.draw(bulletVec6[i]);
				 bulletVec6[i].move(12,-7);
				 if(bulletVec6[i].getGlobalBounds().intersects(player.getGlobalBounds()))
				 {
		...
		if (force == 0)
		{
		 ...
		 if(bulletVec[i].getGlobalBounds().intersects(wrog.getGlobalBounds()))
				 {
					 ...
					 bulletVec[i].setColor(Color::Transparent);
